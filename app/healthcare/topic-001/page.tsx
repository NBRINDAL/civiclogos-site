...
import styles from "../proposal-001/page.module.css";
      ...
      <header className={styles.header}>
        ...
        <div className={styles.hero}>
          <div className={styles.heroCopy}>
            ...
            <h1>{topic001.title}</h1>
            <p className={styles.subtitle}>{topic001.subtitle}</p>
            <p className={styles.thesis}>{topic001.thesis}</p>
          </div>

          <aside className={styles.heroPanel}>
            <span className={styles.panelLabel}>Card note</span>
            <p>{topic001.draftNote}</p>
            <p>{topic001.currentRead}</p>

            <div className={styles.heroMeta}>
              <div>
                <span>Maturity</span>
                <strong>{topic001.maturity}</strong>
              </div>
              <div>
                <span>Revision history</span>
                <strong>{topic001.revisionHistory.length} visible updates</strong>
              </div>
              <div>
                <span>AI roles</span>
                <strong>{topic001.aiPanels.length} active readers</strong>
              </div>
            </div>
          </aside>
        </div>
      ...
      <main className={styles.main}>
        <section className={styles.gridSection}>
          <article className={styles.panel}>
            ...
            <div className={styles.copyBlock}>
              ...
              <p>{topic001.problemStatement}</p>
            </div>

            <div className={styles.copyBlock}>
              <h3>The proposed move</h3>
              <p>{topic001.proposedSolution}</p>
            </div>
          </article>
          ...
          <article className={styles.panel}>
            ...
            <div className={styles.copyBlock}>
              ...
              <div className={styles.tagList}>
                {topic001.stakeholders.map((item) => (
        ...
        <section className={styles.gridSection}>
          <article className={styles.panel}>
            ...
            <div className={styles.copyBlock}>
              ...
              <p>{topic001.strongestObjection}</p>
            </div>

            <div className={styles.copyBlock}>
              <h3>Economic delta</h3>
              <p>{topic001.economicDelta.summary}</p>
              <ul className={styles.bulletList}>
                {topic001.economicDelta.metrics.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            </div>
          ...
          <article className={styles.panel}>
            ...
            <p>{topic001.strongestSupport}</p>

            <div className={styles.evidenceList}>
              {topic001.evidence.map((item) => (
        ...
        <section className={styles.gridSection} id="debate">
          <article className={styles.panel}>
            ...
            <div className={styles.debateGrid}>
              {topic001.debatePrompts.map((item) => (
          ...
          <article className={styles.panel}>
            ...
            <div className={styles.copyBlock}>
              ...
              <ul className={styles.bulletList}>
                {topic001.openQuestions.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            </div>
            ...
            <div className={styles.copyBlock}>
              ...
              <ul className={styles.bulletList}>
                {topic001.whatWouldStrengthen.map((item) => (
                  <li key={item}>{item}</li>
                ))}
              </ul>
            </div>
        ...
        <section className={styles.panel}>
          ...
          <div className={styles.historyList}>
            {topic001.revisionHistory.map((item) => (
